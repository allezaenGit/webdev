  ...
  */
  class Datatables
  {
    ...
    */
	...
	public function order_by($val,$sort)
    {

	  $this->order_by['val'] = $val;
    ...
    */
    ...
    /**
    * Generates the where_post From request client
    * fungsi ini digunakan untuk menambahkan ulang(where original request) 
	* sebelum memanggil fungsi or_where..karena default datatables hanya
	* mencetak (where request POST) di akhir
	* ex
	* 	->where(...);
	*   ->where_post();
	*	->or_where(...);
    */
	public function where_post(){
		$this->get_filtering();
	}
	/**
    * Generates the OR_WHERE portion of the query
    *
    * @param mixed $key_condition
    * @param string $val
    * @param bool $backtick_protect
    ...
    */
    private function get_ordering()
    {
      ...
      if ($this->ci->input->post('order'))
		...
		if($this->order_by){
			$this->ci->db->order_by($this->order_by['val'],$this->order_by['sort']);
		}
    }
    /**
    * Generates a %LIKE% portion of the query
    ...
    */
    private function get_filtering()
    {
	  ...
	  //new 2020.09.27 set searchable false  when data is empty
	  $xx=0;
	  foreach($mColArray as $v){
		  if($v["data"]==""){
			  $mColArray[$xx]["searchable"] = "false";
			  $mColArray[$xx]["orderable"] = "false";
		  }
		$xx++;  
	  }
        ...
        for($i = 0; $i < count($mColArray); $i++){
          if ($mColArray[$i]['searchable'] == 'true' && !array_key_exists($mColArray[$i]['data'], $this->add_columns)){
			...
			//VALUE FILTER COLUMN
			$colsearch = @$mColArray[$i]['search']['value'];
			if($colsearch){
				if($mColArray[$i]['search']['value'] !== '' ){
					
					//EXPLODE FOR MULTI FILTER IN ONE COLUMN
					$acs  = explode(';',$mColArray[$i]['search']['value']);
					if(count($acs)>1){
						 for($ii = 0; $ii < count($acs); $ii++){
							$acsx = $this->ci->db->escape_like_str(trim($acs[$ii]));
							$sWhereC .= $this->select[$mColArray[$i]['data']] . " LIKE '%" . $acsx . "%' OR ";
						 }	 
					}else{
						$acsx = $this->ci->db->escape_like_str(trim($mColArray[$i]['search']['value']));
						$sWhereC .= $this->select[$mColArray[$i]['data']] . " LIKE '%" . $acsx . "%' AND ";
					}
	...
	//  }		
	  ...
	  $sWhereC = substr_replace($sWhereC, '', -4);
	 
	  if($sWhereC != '')
	  $this->ci->db->where('(' . $sWhereC . ')');
    }
    /**
...
', '	', '', '\b', '', '"'), array('\\', '\/', '\n', '\t', '\r', '\b', '\f', '"'));
	...
	/**
     * Inject data post from request API
     * @param int start for start search
	 * @param int length for limit search
	 * @param string find for search value
	 * @param string field field_name table to search the value 
	 *        ex field : name | address
	 *        this condition wil be check for two field table		
     */
	public function injectPost($apiPost){
			$s = @$apiPost["start"] ? $apiPost["start"] : 0 ;
			$l = @$apiPost["length"]? $apiPost["length"]: 10;
			$f = @$apiPost["find"] ;
			$fields = @$apiPost["field"];
...
/* Generated by Harviacode Codeigniter CRUD Generator 2019-06-10 23:59:30 */
/* http://harviacode.com */