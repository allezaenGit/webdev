											...
											{data:null,width:"10%",
												...
												render: function ( data, type, row ) {
														if ( type === 'display' ) {
															var konfirm='';
															var btn_group='';
															btn_group = btn_group + '<button type="button" class="btn btn-default text-red btn-sm shadow single-delete-table " url="<?php echo $link_delete ?>" data="'+row.id+"-"+konfirm+'" tableID="#table-detail" ><i class="fas fa-trash"></i></button></small>';
															...
															btn_group = btn_group + '<button type="button" class="btn btn-default text-red btn-sm shadow btn-send" id="'+row.id+'" url="<?php echo $link_sendmail ?>'+row.id+'" title="send email"><i class="fa fa-send"></i></button>'; 
															return btn_group;
														}	
														return data;	
												}	
...
$('#btn-delete').click(function(){
	deleteCheckedTable('<?php echo $link_delete?>','#table-detail');
});
...
$('#table-detail').on("click",".btn-send",function(){
	...
	a.onSuccess = function(){
		$("#label_send_"+idbutton).html('<label class="label label-sm label-success "  style="font-size:12px">SEND</label>');
	}
	a.onFailed = function(){
			$("#label_send_"+idbutton).html('<label class="label label-sm label-danger " style="font-size:12px">FAILED</label></span>');
	}
