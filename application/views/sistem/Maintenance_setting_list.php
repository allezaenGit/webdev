...
<div class="col-md-12 col-xl-12">
	...
	<div class="form-group">
		<a   class="btn btn-primary shadow" href="<?php echo $link_create?>"><i class="fe fe-check"></i> Create Schedule</a>
		<a   class="btn btn-success shadow" href="<?php echo $link_register_ip?>"><i class="fa fa-address-card"></i> Izinkan IP tertentu - Login saat maintenance mode </a>		
	 </div>
				...
				<?php $x=1; foreach($row as $val){ ?>
					<tr id="<?php echo $val->id ?>">
						...
						<?php if($val->actual_finish==0){?>
							...
							<td><a   href="javascript:void(0)"  class="btn btn-default text-dark btn-sm shadow" title="finish" onclick="stop_maintenance('<?php echo $val->id ?>')"><i class="fa fa-flag"></i></a>
							</td>	
							<?php }else{?>
								<td><?php  echo _indonesia_date($val->actual_finish); ?></td>	
								<td><a href="javascript:void(0)"  class="btn btn-default text-red btn-sm shadow" title="delete" onclick="delete_schedule('<?php echo $val->id ?>')"><i class="fa fa-trash-o"></i></a>
