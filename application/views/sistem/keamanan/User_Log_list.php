...
<div class="panel-body">
		...
		<a href="#" class="btn btn-social btn-danger shadow" id="btn-empty">
                <i class="fas fa-magic"></i> Clear All Data
        </a>
		<br>
		
		<p style="font-size:12px;color:red;margin-top:10px">
							<i class="fe fe-alert-triangle" aria-hidden="true"></i> NOTE :
							 User yang belum logout tidak dapat di hapus permanent</b>, 
		</p>
...
table_detail = $('#table-detail').dataTable({
				...
				columns				:	[	{data:null,width:"5%"},
											...
											{data:null,width:"5%",
												...
												render: function ( data, type, row ) {
														if ( type === 'display' ) {
															 var xx  = '<div class="label label-danger" style="font-size:10px">'+row.total_access+'x<span class="tag-addon"><i class="fe fe-activity"></i></span></div>';		
															 return xx;
														}
											...
											{data:null,width:"10%",
												...
												render: function ( data, type, row ) {
														if ( type === 'display' ) {
															...
															btn_group = btn_group + '<small><a class="btn btn-default text-green btn-sm shadow"  href="<?php echo $link_detail?>'+row.id+'" ><i class="fa fa-search-plus"></i></a></small>';
															return btn_group;
														}	
														return data;	
												}	
...
$('#btn-empty').click(function(){
	...
	emptyTable({
		...
		callback : function (){
			...
			Swal.fire(
				  'Deleted!',
				  'Data has been clear. Logout Now !',
				  'success'
			).then((result) => {
			  
			  if (result.isConfirmed) {
				 window.location.href = "<?= site_url('sistem/Logout')?>";
			  } 
			})
