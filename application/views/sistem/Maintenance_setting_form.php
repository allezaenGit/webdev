	...
	<div class="panel panel-info">
		...
		<div class="row">
			...
			<div class="col-md-2 col-xl-2" > 
				...
				<select name="days" id="select-days" class="form-control select2 data-sending">
				<?php for ($x=0;$x<362;$x++){?>		
			...
			<div class="col-md-2 col-xl-2" > 
				...
				<select name="hours" id="select-hours" class="form-control select2 data-sending"> 
				<?php for ($x=0;$x<24;$x++){?>		
				  <option value="<?php echo $x ?>"><?php if($x<10){$sx="0".$x;}else{$sx=$x;};echo $sx ?></option>  
				<?php }?> 
				</select> 
			...
			<div class="col-md-2 col-xl-2" > 
				...
				<select name="minutes" id="select-minutes" class="form-control select2 data-sending"> 
				<?php for ($x=0;$x<60;$x++){?>		
				  <option value="<?php echo $x ?>"><?php if($x<10){$sx="0".$x;}else{$sx=$x;};echo $sx ?></option>  
				<?php }?> 
		...
		<div class="callout callout-danger shadow">
		  ...
		  <u>WARNING !!!</u><br>
		   1. Page maintenance akan di jalankan , dan hitungan mundur di mulai ketika anda menekan tombol "Simpan dan Jalankan Sekarang"<br>
		...
		<button disabled="" id="btn-save" type="button" class="btn btn-success shadow"><i class="fe fe-save"></i> Simpan dan Jalankan Sekarang</button>	
...
<script> 
	...
	$('.select2').change(function(){
		...
		var a = $('#select-days').select2('data')[0].text + " HARI : " + $('#select-hours').select2('data')[0].text  + " JAM : " +  $('#select-minutes').select2('data')[0].text + " MENIT" 
		$("#l-time").html(a);
	})
