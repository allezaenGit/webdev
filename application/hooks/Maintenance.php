	...
	public function check(){
        ...
        if(!in_array($client_ip,$ips) && $mode == 1 ) {
			...
			if($this->ci->uri->total_segments()==4){
				if($this->ci->uri->segment(1)=="api" && $this->ci->uri->segment(2)=="Public_Access"  ){
					$method = $this->ci->uri->segment(3);
					switch($method){
						case "getFile";
						case APIName::RESPONSE_MAINTENANCE();
						case APIName::RESPONSE_GOOGLE_DRIVE();
							return;	
						break;
					}
				...
				//hanya temporary untuk keperluan registrasi midtrans
				if($this->ci->uri->segment(1)=="api" && $this->ci->uri->segment(2)=="MidtransHook"  ){
					$method = $this->ci->uri->segment(3);
					switch($method){
						case "mntcaXmqpMdcWaoPffGNmzUiadCdBcbdcBqorAuroo";
						case "paymentNotify";
						case "paymentRecurring";
						case "paymentAccount";
						case "paymentUnFinish";
						case "paymentError";
							return;	
						break;
					}
