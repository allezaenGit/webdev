...
class Pengaturan_pengguna extends CI_Controller {
    ...
    public function index() {
		...
		$breadcrumb['Pengaturan Pengguna'] = site_url()."sistem/Pengaturan_pengguna";
	...
	public function create_action(){
		...
		if (!preg_match('/[A-Za-z]/', $val['passuser']) || !preg_match('/[0-9]/',$val['passuser'])){
					$o->success		= 'false';
			 ...
			 if ( !$this->upload->do_upload('inputfile')){
					...
					try{
						...
						foreach($dataFinal as $key){
							...
							$pass = (string) $key["B"] ;
							if(strlen($pass)<6){
									 $dataError[]= "<small>ERROR : ROW (". $x . ") Password Mininal 6 Digit </small><br>";
							}
								
							if (!preg_match('/[A-Za-z]/', $pass) || !preg_match('/[0-9]/',$pass)){
									$dataError[]= "<small>ERROR : ROW (". $x . ") Password harus mengandung huruf dan angka  </small><br>";
							}
					...
					if(count($dataError) > 0){
						...
						$o['success']		= 'true';
						$o['message'] 		= "<small> File Ready in Process,,click save</small><br> <br> 
											
													<a class=\"btn  btn-social btn-info shadow \" onclick=\"save('".site_url()."sistem/Pengaturan_pengguna/create_user_by_template/".$this->_token."')\" href=\"javascript:void(0)\">
