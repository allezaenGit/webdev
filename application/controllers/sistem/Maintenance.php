...
class Maintenance extends CI_Controller { 
	...
	public function maintenance_setting_list(){
			...
			$breadcrumb['Maintenance Page']		= site_url()."sistem/Maintenance/maintenance_setting_list";
			$breadcrumb['List Data']			= "";
			$data = array(
				'breadcrumb'				=> 	$breadcrumb,
				'title_page_big'			=> 	"Maintenance Page",
	...
	public function create (){
		...
		$breadcrumb['sistem']				= site_url()."sistem/Pengaturan";
		$breadcrumb['Maintenance Page']		= site_url()."sistem/Maintenance/maintenance_setting_list";
		$breadcrumb['Create Shecule']			= "";
		
		$data = array(
		'breadcrumb'				=> 	$breadcrumb,
		'title_page_big'			=> 	"Create Shecule",
	...
	public function save_n_run($token){
		if($this->_token ==$token && $this->_user_id==1){
			...
			if($success){
				...
				$f =  '<a href="'.site_url().'sistem/Maintenance/download_urlkey/'.$key .'" type="button" class="btn btn-danger btn-sm"><i class="fa fa-key mr-2"></i>Download key</a>';
	...
	public function download_urlkey($data){
		if($this->_user_id==1){
			$this->_key_register = site_url().'api/Public_Access/' . APIName::RESPONSE_MAINTENANCE() . '/'.$data;
			
			
			$this->load->helper('download');
			force_download('url_key.txt' ,
							'kode ini hanya di gunakan jika anda tidak dapat mengakses halaman login pada mode maintenance'.
							'
