...
class Bot_Telegram_WebHook extends CI_Controller {
   ...
   function __construct(){
	...
	public function index(){
		...
		$breadcrumb['Set & Run Service']	= site_url().'sistem/Bot_Telegram_WebHook';
		
		$menu = $this->load->view('sistem/bot_manager/Telegram_menu',['selected'=> 'run_service'],true);
	...
	public function start_stop_service($id){
		...
		if($data_hook){
			...
			if($data_hook->start==0){
				...
				//start webhook
				$urlhook = base_url(). "bot.php/telegram/" . str_replace("Telegram_","",$d->key) ;
