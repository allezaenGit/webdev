...
class Service_Upload extends CI_Controller {
	...
	public function upload($token){
		if($token==$this->_token){
				...
				$clientConfig 	= @json_decode($this->input->post('clientConfig'));
                ...
                $config['allowed_types']        = 'ogv|wmv|mov|mpg|mp4|avi|mpeg|mp3|wav|png|jpg|jpeg|img|bmp|gif|ico|svg|txt|doc|docx|xls|xlsx|ppt|pptx|pdf|zip|rar|iso|json';
				...
				$data = array();
				 if ( !$this->upload->do_upload('file')){
						...
						if(@$clientConfig->imageResize){
							$file_name = $this->upload->data('file_name');
							$config['image_library']			= 'gd2';
							$config['source_image']				= $path_file . $file_name;
							$config['create_thumb']				= FALSE;
							
							
							$config['maintain_ratio'] = @$clientConfig->imageResize->ratio;
							
							if($clientConfig->imageResize->quality)
								$config['quality']= $clientConfig->imageResize->quality . '%';
							
							if($clientConfig->imageResize->width) 
								$config['width']= $clientConfig->imageResize->width ;
							
							if($clientConfig->imageResize->height)
								$config['height'] = $clientConfig->imageResize->height;
							
							$config['new_image']				= $path_file . $file_name;
						}
						...
						if($autosave=='true'){
							...
							$success  = $this->tmodel->upload_file(	$this->_user_id,
																	...
																	$external = !@$clientConfig->externalUrl ? 0:1);
						}else{
