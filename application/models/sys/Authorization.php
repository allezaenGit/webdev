...
class Authorization extends CI_Model {
	...
	public function get_token($data){
		...
		if(empty($data)){
			...
			return $this->updateTableUserLogin($data->id);
			
		}	
		
	}
	
	public function updateTableUserLogin($iduser){
			$time = time();
	...
	public function getTokenOTP($user){
		$afield1=array(
		 'sys_user.id',	
		 'sys_bot_telegram_client.chat_id'
		);
		$this->db->select($afield1);
		$this->db->where('nmuser', $user);
		$this->db->where('sys_user.opt_status <>', '2');
		$this->db->where('sys_level.opt_status <>', '2');
		$this->db->join('sys_level','sys_level.id=sys_user.opt_level');
		$this->db->join('sys_bot_telegram_client','sys_bot_telegram_client.id_user=sys_user.id');
		$data = $this->db->get('sys_user')->row();
		...
		if(empty($data)){
			...
			$o = new stdclass();
			$o->token = $this->create_token();
			$o->chat_id = $data->chat_id;
			$o->user_id =  $data->id;
			return $o;
		}	
	}
